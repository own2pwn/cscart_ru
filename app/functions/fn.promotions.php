...
function fn_promotion_generate_bonus_coupon($bonus, $bonus_id, $cart)
{
    if (!empty($cart['parent_order_id'])) {
        ...
        $order = fn_promotion_get_order($cart['order_id']);

        if (!empty($order['promotions'][$bonus['promotion_id']]['bonuses'][$bonus_id]['coupon_code'])) {
            return $order['promotions'][$bonus['promotion_id']]['bonuses'][$bonus_id]['coupon_code'];
        }
    }
